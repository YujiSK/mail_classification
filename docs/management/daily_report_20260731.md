...
**記録方法についての訂正（本節を書いた直後に発覚）**: 以下の当初のログは、実装・テスト・実データ実行という複数工程を終えた後にまとめて`date`を1回取得し、同一timestamp（16:35:57）で5行分の結果を一括記入したものだった。これは各工程の実際の開始・完了時刻を表しておらず、ユーザーからの指摘（`project_rules.md` §15を強調・具体化する契機となった）により訂正する。以後は各工程の開始・完了それぞれで実時刻を取得する。本Phase 4分については、記入時点で子作業ごとの実時刻を取得し直していなかったため、実在するGit commit時刻（`git log`の実測値）を境界として使い、内訳は時系列の説明として残す（timestampを工程ごとに捏造しない）。

- 2026-07-31 16:20:53 +07 — **開始**: 直前のPhase 3契約文書commit（`ed9b19f`）完了を受けて、ユーザー指示に基づきPhase 4（Core実験実行・OOF予測・指標集計）へ着手した。手順: (1) execution_plan.md等のPhase 4 statusを更新、(2) `src/mail_classification/`配下に実験実行・OOF・集計モジュールを実装、(3) 確定済みCore条件D0〜D2×2モデル＝6セルを共通Fold（`outputs/folds/common_folds.json`）で5-fold CV・OOF・指標集計・保存、(4) test実施・`git diff`確認・日報・実行計画同期・commitまで自動で実施。
- 2026-07-31 16:37:38 +07 — **完了 (所要時間: 約17分、commit `9a6cf25`の実時刻を終点として実測)**: 上記(1)〜(3)を実施した。内訳（この17分間の実施順序）:
  1. task9の`evaluate_pipeline_cv`/`summarize_cv`/`paired_fold_diff`設計（`docs/audits/task10_reuse_matrix.md`でB＝Adapt and reuse評価済み）を参考に、`src/mail_classification/evaluation/`へ`cv.py`（`run_core_cell`/`run_core_experiments`：Fold毎にfit/predict、OOF行・timing・vocabulary_sizeを収集）、`metrics.py`（`build_metrics_long`：accuracy/macro_f1/weighted_f1/classwise P・R・F1、`build_confusion_matrix_rows`）、`aggregate.py`（`build_metrics_summary`：CV mean/std/n_folds）、`paired.py`（`build_paired_differences`：D0基準のFold単位ペア差、対応Fold欠落時は`ValueError`）、`runner.py`（`load_fold_artifact`/`run_and_write_core_experiments`）を実装した。task9はpandasを使用するが、task10既存モジュール（`quality/statistics.py`等）がstdlib（`statistics`/`collections.Counter`/`csv`）のみで完結している方針に合わせ、pandasは導入しなかった。
  2. `RunManifest`は元々単一モデル用schema（`model_name`は`str | None`単数）のため、6モデルを跨ぐ1つのCore run manifestでは`model_name`/`model_parameters`を`null`とし、Phase 2の非モデルmanifestと同じ扱いにした。各セルの内訳は`metrics_long.csv`等の`condition`/`model`列で識別する。`fold_artifact_path`/`fold_artifact_hash`は`RunManifest`既存fieldへ実際に記録し、Phase 3で「Phase 4で対応」としていた項目を解消した。
  3. `tests/test_cv.py`4件、`tests/test_metrics.py`6件、`tests/test_aggregate.py`3件、`tests/test_paired.py`3件、`tests/test_runner.py`5件（合成データでのcoverage/非負timing/vocabulary、実データ800件での6セル end-to-endを含む）を追加。全test`194 passed`、`uv lock --check`・`git diff --check`成功。
  4. テストに加え、実データ（Full 800件、既存の`outputs/folds/common_folds.json`）に対して実際に`run_and_write_core_experiments`を実行し、`outputs/runs/phase4-core-seed42/`へ6セル分のFold Long（450行＝6セル×5fold×15metric）、集約、OOF（4800行＝6セル×800件、各セルで800件1:1 coverage確認）、confusion matrix、paired differences、manifestを生成した。実測macro-F1 cv_mean: D0 linear_svc 0.616、D0 logistic_regression 0.593、D1 linear_svc 0.625、D1 logistic_regression 0.575、D2 linear_svc 0.596、D2 logistic_regression 0.610（fold間std 0.08〜0.13）。生成物は再現可能なため`outputs/runs/`を`.gitignore`へ追加してGit非追跡とした。結果の原因分析・優劣判断はPhase 5（説明性・誤分類分析）の役割とし、本Phaseでは断定していない。
  実装をcommit `9a6cf25`として記録した。
- 2026-07-31 16:37:38 +07 — **開始**: execution_plan.mdのPhase 4 Status/Main tasks/Main outputs/直近アクション/未確認事項を実施済みへ同期し、冒頭snapshot表のPhase 4行を更新する。
- 2026-07-31 16:38:09 +07 — **完了 (所要時間: 1分未満、commit `8877d9d`)**: execution_plan.mdを同期し、Phase 4を`Completed`とした。commit hashを追記する小commitを作成。
