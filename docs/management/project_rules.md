...
**本節は本プロジェクトの最重要規約の一つとする。日報記録を後回しにしたり、複数工程をまとめて末尾に一括記入することは、規約違反として扱う。**

- 工程（実装・テスト・重要判断のいずれか意味のある単位）を開始する瞬間に、その場で`date`コマンド等により実時刻を取得し、`YYYY-MM-DD HH:MM:SS +07 — **開始**: [対象範囲]`を`docs/management/daily_report_20260731.md`（または後続日報）へ即時追記する。
- 同じ工程が完了した瞬間に、再度実時刻を取得し、`YYYY-MM-DD HH:MM:SS +07 — **完了 (所要時間: XX分)**: [成果・検証結果]`を追記する。所要時間は開始時刻からの実測差分とする。
- **禁止事項**: 複数の工程をまとめて実施した後、まとめて一つの（または同一の）timestampで日報へ書き下すこと。これは各工程の実際の開始・終了時刻を記録したものではなく、追跡可能性（traceability）の目的を損なう。
- 一つの工程内で複数の子作業・判断がある場合も、それぞれの区切りで独立した開始・完了ペアを記録するか、少なくとも完了時に子作業ごとの実施順序と実測所要時間の目安を明記する。
