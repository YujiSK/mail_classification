"""Phase 4 Core run orchestration: fit/predict every cell, write every artifact.

One run_id covers the whole Core batch (every condition x model cell on the
same Fold artifact), matching docs/architecture/task10_architecture.md's
``outputs/runs/<run_id>/`` layout. The manifest's ``model_name``/
``model_parameters`` are null (as Phase 2's non-model manifests already do)
because a single manifest here covers multiple models; each cell is
distinguished by the ``condition``/``model`` columns in the CSV artifacts.
"""
...
from .cv import run_core_experiments
from .metrics import (
    CONFUSION_FIELDS,
    METRICS_LONG_FIELDS,
...
def run_and_write_core_experiments(
    ...
    *,
    ...
    """Run every Core cell, write all Phase 4 artifacts, return the run directory."""
    ...
    paired_rows = build_paired_differences(metrics_long)
    oof_rows = [
        {
    ...
    resolved_run_id = run_id or f"phase4-core-seed{fold_artifact.metadata.random_seed}"
    ...
    manifest = RunManifest(
        ...
        command=["run_core_experiments", *conditions, *models],
        python_version=platform.python_version(),
        platform=platform.platform(),
        dependency_versions={
            package: version(package) for package in ("scikit-learn", "pydantic")
        },
        config_path=None,
        config_hash=None,
        data_path=None,
