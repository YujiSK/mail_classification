"""Phase 5 explainability run orchestration: coefficients + misclassification analysis.

Consumes Phase 4's already-written ``predictions_oof.csv`` rather than
recomputing OOF, so the explained errors are traceably the exact predictions
that were evaluated, not a fresh (deterministic but separate) recomputation.
"""
...
from .evidence import enrich_misclassifications_with_evidence
from .linear import (
    COEFFICIENT_FIELDS,
...
def read_oof_predictions(path: str | Path) -> list[dict[str, object]]:
    """Read a Phase 4 predictions_oof.csv back into typed dict rows."""
...
def run_and_write_explainability(
    ...
    *,
    ...
    misclassification_rows = build_misclassification_rows(relevant_oof_rows, records_by_id)
    ...
    resolved_run_id = run_id or f"phase5-explain-seed{fold_artifact.metadata.random_seed}"
    ...
    write_csv(
        run_dir / "misclassifications.csv", misclassification_rows, MISCLASSIFICATION_FIELDS
    )
    write_csv(
    ...
    manifest = RunManifest(
        ...
        command=["run_and_write_explainability", *conditions, *models],
