"""Japanese counterpart of ``pipeline.py``.

Orchestration is identical to the English stage runner; only the generator
class, template loader, quality-audit modules (Sudachi-based instead of the
English-regex-based ones), default config path, run-ID prefix
(``phaseJA2-*`` instead of ``phase2-*``), and manifest
``preprocessor_name``/dependency versions differ. Writes to
``configs/phase2_ja.yml``'s JA-suffixed data/manifest paths only, never to
the English track's paths (``docs/audits/task10_ja_reuse_matrix.md``).
"""
...
from .ja_models import load_ja_template_catalog
...
def run_ja_generation_stage(
    stage: str,
    root: str | Path,
    config_path: str | Path = "configs/phase2_ja.yml",
) -> JaStageResult:
    ...
    catalog = load_ja_template_catalog(template_path)
    generator = JapaneseSyntheticMailGenerator(config, catalog)
    ...
    summary_path = quality_dir / f"{stage}_summary_ja.json"
    duplicate_counts = {
        match_type: sum(
    ...
    if stage in {"pilot", "full"}:
        write_csv(
            quality_dir / f"{stage}_duplicates_ja.csv",
            duplicates,
            DUPLICATE_FIELDS,
        )
        write_csv(
            quality_dir / f"{stage}_leakage_findings_ja.csv",
            leakage,
            LEAKAGE_FIELDS,
        )
        review_rows = (
        ...
        write_csv(
            quality_dir / f"{stage}_review_samples_ja.csv",
            review_rows,
            REVIEW_FIELDS,
        )

    run_id = f"phaseJA2-{stage}-seed{config.generator.seed}"
    manifest_path = project_root / config.paths.manifest_dir / f"{run_id}.json"
    dependency_versions = {
        package: version(package)
        for package in ("pydantic", "pyyaml", "sudachipy", "sudachidict-core", "neologdn")
    }
...
def _validate_pilot_approval(
    decision_path: Path,
    generator: JapaneseSyntheticMailGenerator,
    template_path: Path,
