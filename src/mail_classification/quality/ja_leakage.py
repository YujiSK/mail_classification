"""Japanese counterpart of ``leakage.py``.

Two changes from the English module: (1) ``HEADER_VALUE`` matches the
Japanese header labels this track's generator/cleaner use (``差出人``/
``件名``) instead of ``from``/``subject``; (2) exclusive-lexical-feature
scanning tokenizes with ``JapanesePreprocessor`` (Sudachi) instead of the
English regex ``TOKEN_PATTERN``, since Japanese has no inter-word spaces for
a regex word-boundary tokenizer to exploit. Everything else -- finding
categories, thresholds, sort order -- is identical to ``leakage.py``.
"""
...
HEADER_VALUE = re.compile(r"(?im)^(差出人|件名):\s*(.+)$")
...
def _audit_header_and_signature(records, findings, minimum) -> None:
    ...
    for record in records:
        for kind, value in HEADER_VALUE.findall(record.raw_text):
            ...
            if kind == "差出人" and "@" in normalized:
                normalized = normalized.rsplit("@", 1)[1]
                kind = "sender_domain"
            values[(kind, normalized)].append(record)
...
def _audit_exclusive_lexical(records, findings, minimum) -> None:
    ...
    values: dict[tuple[str, str], list[RawMailRecord]] = defaultdict(list)
    for record in records:
        tokens = list(preprocessor.transform(record.body_text).tokens)
