"""生成済みHTMLをChrome/Chromium headlessでPDF化する。"""
...
def render_html_to_pdf(
    ...
    document_root = (document_root or OUTPUTS_DIR).resolve()
    if document_root not in html_path.parents and html_path.parent != document_root:
        raise ValueError(
            f"html_path must live under {document_root} so relative image paths resolve: "
            f"{html_path}"
        )
    if CHROME_BIN is None:
        raise RuntimeError(
            "Chrome/Chromium was not found. Install google-chrome, chromium, or chromium-browser."
        )
