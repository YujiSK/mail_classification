...
DEFAULT_MD_PATH = PROJECT_ROOT / "outputs" / "SUMMARY_REPORT.md"
DEFAULT_CSS_PATH = PROJECT_ROOT / "tools" / "pdf_renderer" / "assets" / "styles" / "report.css"
DEFAULT_BUILD_DIR = PROJECT_ROOT / "outputs" / "renders" / "_build"
...
def build(
    ...
    base_uri = md_path.resolve().parent.as_uri().rstrip("/") + "/"
    title_tag = soup.find("h1")
    document_title = _visible_text(title_tag) if title_tag else md_path.stem
    full_html = (
        "<!doctype html>\n"
        '<html lang="ja">\n<head>\n<meta charset="utf-8">\n'
